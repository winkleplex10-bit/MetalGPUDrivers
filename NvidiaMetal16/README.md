...
**Series folder:** Turing TU11x (GTX 1630 / 1650 / 1660). The GOP kext still matches every NVIDIA VGA device (vendor `10DE`, class `0x03`) so an unknown ID cannot fall through to Kepler `NVDAStartup`. Inject **only one** `NvidiaMetalXX.kext`.
...
Output: `build/NvidiaMetal16.kext`
...
Logs: `NvidiaMetal16:` in `log show --predicate 'process == "kernel"' --last boot` or `-v` boot.

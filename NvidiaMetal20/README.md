...
**Series folder:** Turing TU10x (RTX 2060 / 2070 / 2080). The GOP kext still matches every NVIDIA VGA device (vendor `10DE`, class `0x03`) so an unknown ID cannot fall through to Kepler `NVDAStartup`. Inject **only one** `NvidiaMetalXX.kext`.
...
Output: `build/NvidiaMetal20.kext`
...
Logs: `NvidiaMetal20:` in `log show --predicate 'process == "kernel"' --last boot` or `-v` boot.

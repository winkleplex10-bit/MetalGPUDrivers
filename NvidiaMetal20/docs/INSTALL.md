# Installing NvidiaMetal20 (unaccelerated GOP display)
...
## 2. Inject NvidiaMetal20.kext

Copy `build/NvidiaMetal20.kext` to `EFI/OC/Kexts/` and add:
...
<dict>
	...
	<string>x86_64</string>
	<key>BundlePath</key>
	<string>NvidiaMetal20.kext</string>
	<key>Enabled</key>
	<true/>
	<key>ExecutablePath</key>
	<string>Contents/MacOS/NvidiaMetal20</string>
	<key>PlistPath</key>
...
```bash
log show --last boot --predicate 'eventMessage CONTAINS "NvidiaMetal20"'
ioreg -l -w0 | grep -A20 NvidiaGopFramebuffer20
```

You want `NvidiaGopFramebuffer20` under the NVIDIA `IOPCIDevice`, **no** `NVDAStartup` / `NVDA` children, and `AAPL,boot-display` if this card is the console.

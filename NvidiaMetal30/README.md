...
**Series folder:** Ampere GA10x (RTX 3050 / 3060 / 3070 / 3080 / 3090). The GOP kext still matches every NVIDIA VGA device (vendor `10DE`, class `0x03`) so an unknown ID cannot fall through to Kepler `NVDAStartup`. Inject **only one** `NvidiaMetalXX.kext`.
...
Output: `build/NvidiaMetal30.kext`
...
Logs: `NvidiaMetal30:` in `log show --predicate 'process == "kernel"' --last boot` or `-v` boot.

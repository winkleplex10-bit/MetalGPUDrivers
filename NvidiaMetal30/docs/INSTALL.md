# Installing NvidiaMetal30 (unaccelerated GOP display)
...
## 2. Inject NvidiaMetal30.kext

Copy `build/NvidiaMetal30.kext` to `EFI/OC/Kexts/` and add:
...
<dict>
	...
	<string>x86_64</string>
	<key>BundlePath</key>
	<string>NvidiaMetal30.kext</string>
	<key>Enabled</key>
	<true/>
	<key>ExecutablePath</key>
	<string>Contents/MacOS/NvidiaMetal30</string>
	<key>PlistPath</key>
...
```bash
log show --last boot --predicate 'eventMessage CONTAINS "NvidiaMetal30"'
ioreg -l -w0 | grep -A20 NvidiaGopFramebuffer30
```

You want `NvidiaGopFramebuffer30` under the NVIDIA `IOPCIDevice`, **no** `NVDAStartup` / `NVDA` children, and `AAPL,boot-display` if this card is the console.

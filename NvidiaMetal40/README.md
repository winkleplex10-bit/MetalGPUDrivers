...
**Series folder:** Ada Lovelace AD10x (RTX 4050 / 4060 / 4070 / 4080 / 4090). The GOP kext still matches every NVIDIA VGA device (vendor `10DE`, class `0x03`) so an unknown ID cannot fall through to Kepler `NVDAStartup`. Inject **only one** `NvidiaMetalXX.kext`.
...
Output: `build/NvidiaMetal40.kext`
...
Logs: `NvidiaMetal40:` in `log show --predicate 'process == "kernel"' --last boot` or `-v` boot.

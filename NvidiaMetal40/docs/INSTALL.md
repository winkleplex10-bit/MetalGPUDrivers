# Installing NvidiaMetal40 (unaccelerated GOP display)
...
## 2. Inject NvidiaMetal40.kext

Copy `build/NvidiaMetal40.kext` to `EFI/OC/Kexts/` and add:
...
<dict>
	...
	<string>x86_64</string>
	<key>BundlePath</key>
	<string>NvidiaMetal40.kext</string>
	<key>Enabled</key>
	<true/>
	<key>ExecutablePath</key>
	<string>Contents/MacOS/NvidiaMetal40</string>
	<key>PlistPath</key>
...
```bash
log show --last boot --predicate 'eventMessage CONTAINS "NvidiaMetal40"'
ioreg -l -w0 | grep -A20 NvidiaGopFramebuffer40
```

You want `NvidiaGopFramebuffer40` under the NVIDIA `IOPCIDevice`, **no** `NVDAStartup` / `NVDA` children, and `AAPL,boot-display` if this card is the console.

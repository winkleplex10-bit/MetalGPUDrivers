...
#define NVFB_LOG(fmt, ...) \
    IOLog("NvidiaMetal50: " fmt "\n", ##__VA_ARGS__)

OSDefineMetaClassAndStructors(NvidiaGopFramebuffer, IOFramebuffer);
...
bool NvidiaGopFramebuffer::parseBootVideo()
{
...
bool NvidiaGopFramebuffer::gopLivesInThisDevice()
{
...
void NvidiaGopFramebuffer::publishIdentity()
{
    if (fPCI == nullptr) {
        return;
    }
    ...
    fPCI->setProperty("NVDAType", "NvidiaMetal50");
...
void NvidiaGopFramebuffer::fillPixelInfo(IOPixelInformation *pixelInfo) const
{
...
bool NvidiaGopFramebuffer::start(IOService *provider)
{
...
void NvidiaGopFramebuffer::stop(IOService *provider)
{
    fPCI = nullptr;
    fHaveGop = false;
    fIsConsole = false;
...
void NvidiaGopFramebuffer::free()
{
    super::free();
}

IOReturn NvidiaGopFramebuffer::enableController()
{
    if (!fHaveGop) {
...
bool NvidiaGopFramebuffer::isConsoleDevice()
{
    return fIsConsole;
}

IODeviceMemory *NvidiaGopFramebuffer::getApertureRange(IOPixelAperture aperture)
{
...
IODeviceMemory *NvidiaGopFramebuffer::getVRAMRange()
{
...
const char *NvidiaGopFramebuffer::getPixelFormats()
{
    return (fDepth == 16) ? IO16BitDirectPixels : IO32BitDirectPixels;
}

IOItemCount NvidiaGopFramebuffer::getDisplayModeCount()
{
    return fHaveGop ? 1 : 0;
}

IOReturn NvidiaGopFramebuffer::getDisplayModes(IODisplayModeID *allDisplayModes)
{
...
IOReturn NvidiaGopFramebuffer::getInformationForDisplayMode(
...
UInt64 NvidiaGopFramebuffer::getPixelFormatsForDisplayMode(
...
IOReturn NvidiaGopFramebuffer::getPixelInformation(
    IODisplayModeID displayMode, IOIndex depth,
...
IOReturn NvidiaGopFramebuffer::getCurrentDisplayMode(IODisplayModeID *displayMode,
                                                    IOIndex *depth)
{
...
IOReturn NvidiaGopFramebuffer::setDisplayMode(IODisplayModeID displayMode,
                                             IOIndex depth)
{
...
IOReturn NvidiaGopFramebuffer::getStartupDisplayMode(IODisplayModeID *displayMode,
                                                    IOIndex *depth)
{
    return getCurrentDisplayMode(displayMode, depth);
}

IOReturn NvidiaGopFramebuffer::getTimingInfoForDisplayMode(
...
IOItemCount NvidiaGopFramebuffer::getConnectionCount()
{
    return 1;
}

IOReturn NvidiaGopFramebuffer::getAttribute(IOSelect attribute, uintptr_t *value)
{
...
IOReturn NvidiaGopFramebuffer::setAttribute(IOSelect attribute, uintptr_t value)
{
...
IOReturn NvidiaGopFramebuffer::getAttributeForConnection(
...
IOReturn NvidiaGopFramebuffer::connectFlags(IOIndex connectIndex,

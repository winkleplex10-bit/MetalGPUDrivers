...
**Binding project rules:** [BUILD-RULES.md](BUILD-RULES.md) — **macOS 26** ship pin, **WhateverGreen absent**, Apple Silicon display track is in-scope under `ihv/apple-silicon/` (not this folder).  
...
**Hard rules.** Follow [BUILD-RULES.md](BUILD-RULES.md). No SIP / OpenCore / AuxKC / unsigned-kext *recipes*. Lab fact (not a recipe): AuxKC came from **`/Library/Extensions`** (not `/tmp`); runtime `kmutil load` is too late vs IONDRV. No Lilu / NootedRed / WhateverGreen *patch recipes*. Product stack has **WhateverGreen absent**; if WEG appears on another install, remain DID-only compatible (§2.1). **No X6000 personality spoof or Info.plist injection onto Raphael DID `0x164E`** — match-level fake-id is not a substitute for this IHV (§0.2). If a selector, entitlement, bundle ID, IOGPU method, PM4 opcode, or firmware RPC is not in the research corpus, a public header you have opened, or a URL cited here, write **UNKNOWN** and stop that branch.
...
**Project policy:** [BUILD-RULES.md](BUILD-RULES.md) — product OS is **macOS 26**, **WhateverGreen absent**. Tahoe lab matches that pin (no WEG). If WEG appears on another install, remain DID-only and do not race its patch sites.
...
| WhateverGreen | **Absent** (product / Tahoe lab) | **Do not patch, require, or depend on.** | Compatible by non-overlap if present elsewhere |
...
Host: **AMD Hackintosh x86** with Raphael CPU and iGPU enabled in firmware (GOP → APU heads). Discrete GPU may be installed. Product config: **WhateverGreen absent** ([BUILD-RULES.md](BUILD-RULES.md)). Intel Mac Pro is the wrong host for this slot (no Raphael package). Apple Silicon / DCP is **not this folder** — that track lives in `ihv/apple-silicon/` and is in-scope for the repo.
...
4. NootedRed out of product code; dGPU coexistence in scope; WEG **absent** (product) — **done**.  
...
4. **Do not install or depend on WhateverGreen** for this IHV (product = WEG absent). Do not require disabling the discrete GPU.  

...
**Hard rules.** Architecture and implementation plan only. No SIP / OpenCore / AuxKC / unsigned-kext recipes. No Lilu / NootedRed / WhateverGreen patch recipes. No X6000 personality spoof onto Raphael DID `0x164E`. If a selector, entitlement, bundle ID, IOGPU method, PM4 opcode, or firmware RPC is not in the research corpus, a public header you have opened, or a URL cited here, write **UNKNOWN** and stop that branch.

---

## 1. Goal

Build an **unofficial IHV-quality GPU stack** so the **Raphael RDNA2 iGPU** (and later Rembrandt) can do **display + Metal** on a Hackintosh-on-AMD-APU machine where kexts can be loaded for development.

**Done** = WindowServer desktop on our `IOFramebuffer`, and `MTLCopyAllDevices()` returns our GPU running a stock `.metallib`. Spoofing Raphael as Navi 21 so `AMDRadeonX6000` attaches, or extending NootedRed with Lilu patches, are **not-done**.

**Stance:** Hard, but closer than RDNA3 dGPU in one respect: Apple’s live discrete Metal stack **is** RDNA2 (`AMDRadeonX6000`). The new work is **APU form factor** (UMA, DCN 3.1.5, 2 CU, APU match), not inventing a missing GFX generation. Still a **new IHV backend** — APU DCN/UMA ≠ Navi 21 FB.
...
Host: **AMD Hackintosh x86** with Raphael CPU and iGPU enabled in firmware (CSM/GOP → APU heads). Intel Mac Pro is the wrong host for this slot (no Raphael package). macOS pin: **26 Tahoe** (last major Intel macOS), same as other IHV slots. Apple Silicon / DCP replacement = out of scope.
...
| **Temptation to “just NootedRed it”** | Same ISA gen as Apple → spoof feels close. | Repo hard rule: no X6000 spoof. Trace ABI; implement IHV. |
...
Freeze: AMD AM5 host with **7950X3D** (or another Raphael SKU), iGPU enabled, Tahoe build, project bundle IDs (not `com.apple.*`). Capture:

1. Raphael PCI/ACPI identity (`1002:164E`, rev, ACPI path).  
2. Working **X6000** IORegistry + `MTLCopyAllDevices` on the **same** Tahoe build (ABI oracle — may be a second machine with a supported dGPU).  
3. Firmware name list for GC 10.3.6 / DCN 3.1.5 / MP0 13.0.5 and **license UNKNOWN** status.  
4. Written statement: NootedRed out of scope for product code.

**Accept:** `ihv/amd-rdna2-igpu/docs/boards.md` + `unknowns.md`. No code required beyond stubs.
...
**Accept:** R1.1 nub trained. R1.2 our `IOClass`. R1.3 VID/DID `1002:164E` readable. R1.4 firmware ready **or** UNKNOWN+license block (no 3D bit-bang). R1.5 no-op completes. R1.6 invisible to Metal.

### Phase R2 — dumb framebuffer

`IOFramebuffer` subclass; modeset on APU outputs; WindowServer desktop. No Metal.

**Accept:** R2.1 FB + console/panic. R2.2 login/desktop visible. R2.3 cursor/VBL stubbed. R2.4 unaccelerated OK.
...
**Accept:** R5.1–R5.5 as host Phase 5 (see `CURSOR-IHV-DRIVER-SPEC` when present; until then mirror [CURSOR-START-AMD.md](CURSOR-START-AMD.md) §7 Phase 5).

### Phase R6 — present

`CAMetalLayer` + `presentDrawable` on the R2 FB. `CGDirectDisplayCopyCurrentMetalDevice` returns us.

**Accept:** R6.1–R6.4 as host present criteria.
...
ihv/amd-rdna2-igpu/           # NootedRed-gap RDNA2 APU — this slot
  ...
  match/                      # Raphael 0x164E (+ later Rembrandt); APU/platform match
  display/                    # DCN 3.1.5 + APU connectors
  submit/                     # PM4 / GFX10.3 compute
  isa/                        # AIR → gfx1036 / gfx1035
  uma/                        # Shared / IOSurface rules
  docs/                       # boards.md, unknowns.md, traces
```

`host/` must stay vendor-agnostic. Adding this folder must not edit `ihv/amd-rdna3/` ISA or NootedRed sources (NootedRed is not in-tree).
...
2. **Do not fork NootedRed, Lilu, or WhateverGreen into product code.** Cite them as gap/prior art only.  
3. **Do not claim NootedRed “will support RDNA2 soon” as a substitute for this IHV.**  
4. **Do not put Vega / 7x30 / Phoenix / Strix into this folder.**  
5. **Do not write SIP, OpenCore, AuxKC, or unsigned-kext load steps.** If attach fails: IORegistry dump and stop.  
6. **Do not flash unsigned PSP/SMU/GC firmware.** License UNKNOWN → stop.  
7. **Do not invent IOGPU selectors or AIR opcodes.** UNKNOWN + cite.  
8. **Do not treat Intel Mac Pro PCIe as the Raphael bring-up host.**  
9. **Never say impossible.** Say what is hard, why, and which phase attacks it.
...
8. Interaction when a discrete unsupported GPU (e.g. Nvidia) is also present — primary display ownership UNKNOWN until R0.

---

## 11. Suggested execution order (after folder exists)

1. **Write `docs/boards.md`** for the 7950X3D (or chosen Raphael) — VID/DID/rev, connectors, OS build, firmware list.  
2. **Capture X6000 oracle** on the same Tahoe build (second machine OK).  
3. **File `unknowns.md`** from §10; mark license blockers.  
4. **Implement `match/` stub** personality only (no display programming).  
5. **R1 firmware heartbeat** using public register/firmware docs — stop on license.  
6. **R2 dumb FB** before any Metal.  
7. **R3–R6** only after R2 green; reuse `host/` shells proven by RDNA3 where available.  
8. Keep NootedRed as external reference only; never as a git submodule for product.

---

## 12. Sources

**Gap / prior art:** [NootedRed](https://chefkiss.dev/applehax/nootedred/); [discussion #345 — 7950X / RDNA2 unsupported](https://github.com/ChefKissInc/NootedRed/discussions/345); [amd-osx thread](https://forum.amd-osx.com/threads/nootedred-kext-and-7950x.5780/).
...
**Negative methodology:** WhateverRed / WhateverGreen — blob enablement dies when the required generation logic is absent; for Raphael the ISA generation exists, but APU enablement by DID spoof remains out of product policy for this repo.

...
**Status:** PCI/ACPI identity from Sequoia dump `docs/traces/sequoia-7950x3d/` (2026-09-06). **Bring-up OS: macOS 26 Tahoe, no WhateverGreen, build 25G83.** Sequoia traces are historical. Living plan: [`docs/CURSOR-START-AMD-RDNA2-IGPU.md`](../../../docs/CURSOR-START-AMD-RDNA2-IGPU.md) §0.
...
| Current FB | **Tahoe 0.2.8/0.2.10:** `RaphaelFramebuffer` (`R2-gop-wrap`, `connector-kind=DP`) on `VGA@0`; controller `RaphaelPhase=R2-dcn-modeset`, `RaphaelDmubOk=Yes`. **0.2.9** V_TOTAL+1 live on DP blacked the console. Pre-wrap / late load: `IONDRVFramebuffer` (`.Display_boot`). |
...
| First kexts | `dev.metalgpudrivers.RaphaelIGPU` + `dev.metalgpudrivers.RaphaelFB` **0.2.10** — see [BUILD.md](BUILD.md). AuxKC from `/Library/Extensions` (lab fact). |

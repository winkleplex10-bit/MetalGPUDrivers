...
This kext’s **0.2.10** drop wraps GOP scanout by default. `raphael_dcn_modeset=1` loads the host copy of `dcn_3_1_5_dmcub.bin`, GPINTs GOP DMCUB even when `dal_fw=0`, **reaffirms** the live OTG0 3840×2160 pipe (physical **DP** jack), then HUBP blank/unblank. That is not a new PLL. 0.2.9 left `V_TOTAL+1` live and lost the DP sink; do not do that on the lab boot-arg. Next slice: cited DP stream/MSA or DIG, still no firmware reverse.
